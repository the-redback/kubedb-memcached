...
func (c *Controller) ValidateSnapshot(snapshot *tapi.Snapshot) error {
	...
	if _, err := c.ExtClient.Memcacheds(snapshot.Namespace).Get(databaseName, metav1.GetOptions{}); err != nil {
		return err
	}
...
func (c *Controller) GetDatabase(snapshot *tapi.Snapshot) (runtime.Object, error) {
	memcached, err := c.ExtClient.Memcacheds(snapshot.Namespace).Get(snapshot.Spec.DatabaseName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (c *Controller) GetSnapshotter(snapshot *tapi.Snapshot) (*batch.Job, error) {
	...
	memcached, err := c.ExtClient.Memcacheds(snapshot.Namespace).Get(databaseName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}

	// Get PersistentVolume object for Backup Util pod.
	persistentVolume, err := c.getVolumeForSnapshot(memcached.Spec.Storage, jobName, snapshot.Namespace)
	if err != nil {
		return nil, err
	}
					...
					Containers: []core.Container{
						{
							...
							Image: fmt.Sprintf("%s:%s-util", docker.ImageMemcached, memcached.Spec.Version),
							Args: []string{
