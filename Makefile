...
DOCKERFILE_PROD  = hack/docker/mc-operator/Dockerfile.in
DOCKERFILE_DBG   = hack/docker/mc-operator/Dockerfile.dbg
...
install:
	@cd ../installer; \
	APPSCODE_ENV=dev KUBEDB_DOCKER_REGISTRY=$(REGISTRY) KUBEDB_OPERATOR_TAG=$(TAG) KUBEDB_CATALOG=memcached ./deploy/kubedb.sh --operator-name=$(BIN)
